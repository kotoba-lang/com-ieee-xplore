(ns ieee_xplore.main
  "Kotodama WASM entrypoint for the Ieee_xplore clean-room actor (L4) — Clojure port.
  ...
  facts (`ieee_xplore.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ieee_xplore")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"      :plural "works"      :id-prefix "ieeexplo_wor"
    :fields [:doi :title :type :publishedAt]            :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"    :plural "authors"    :id-prefix "ieeexplo_aut"
    :fields [:orcid :givenName :familyName]             :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"  :plural "citations"  :id-prefix "ieeexplo_cit"
    :fields [:workId :citedDoi]                         :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "ieeexplo_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"    :plural "records"    :id-prefix "ieeexplo_rec"
    :fields [:accession :database :title]               :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"   :plural "subjects"   :id-prefix "ieeexplo_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"ieee_xplore.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ieee_xplore-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ieee_xplore-compat/ready)
